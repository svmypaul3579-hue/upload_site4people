...
def handle_selenium_error(driver, exception, context_msg):
    print(f"Error caught: {context_msg} - {exception}")
...
def modify_csv_data(data, row_id=None, new_row_data=None):
    ...
    if new_row_data is None:
        print("No new row data provided.")
        return df
    ...
    if row_id is not None:
        ...
        if target_row is not None:
            ...
            print(f"Successfully updated row index: {target_row}")
        else:
            print(f"Error: Row ID {row_id} does not exist.")
    else:
...
try:
    ...
    for index, row in leads_df.iterrows():
        ...
        print(f"Processing row {index}: Company Name: {company_name}, HR Name: {name}, First Name: {first_name}, Last Name: {last_name}")
        ...
        if not existing_user.empty:
            ...
            print(f"Company '{company_name}' already exists. Fetched username: {username}")
        else:
            # Register user
            ...
            # print(result)
            if result is False:
                print(f"Registration failed for username: {username}. Skipping to next row.")
        ...
        try:
            ...
            print(f"Login failed for username: {username}. Error: {e}")
